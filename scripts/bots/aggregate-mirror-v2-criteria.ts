...
function buildMarkdown(fellows: FellowAggregate[], generated: string): string {
  const lines: string[] = [
...
function writeIndexCsv(pathStr: string, fellows: FellowAggregate[]): void {
  ...
  for (const f of fellows) {
    ...
    const full = f.constitution?.length ?? 0;
    const total =
...
function main(): void {
  ...
  fs.writeFileSync(absMd, buildMarkdown(fellows, generated), "utf-8");
