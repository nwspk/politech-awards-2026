...
| Open | Evaluation | How should Alexandra D1–D8 jury outputs feed awards (shortlist only vs `SCORING_MODE=v10` ranking)? | v10 adds numeric transparency; weights and category corrections still optional | Run pilot `npm run alexandra-eval -- --limit N`; compare spread to ITN/A; decide publication policy |
