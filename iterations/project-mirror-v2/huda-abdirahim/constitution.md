...
- **Weight:** 20 points
- **Why Huda:** "Collective ownership of resources should be technically legible and democratically governed" — the constitutional core of the bio. [bio, CONFIRMED]
...
- **Applies when:** Project uses programmable, on-chain, or cryptographically verifiable mechanisms to enforce transparency or governance accountability. Mechanism must be functional, not aspirational.
- **Why Huda:** "Programmable governance" is explicitly listed as an interest. [bio, CONFIRMED]
...
**Potential conflict: Modifier 1 (programmable/on-chain governance boost) applied to mainstream civic tech**

Modifier 1 is designed for projects that use cryptographic/on-chain mechanisms. Most civic tech projects (mySociety, Decidim, voter registration tools) do not use on-chain mechanisms. The modifier should not fire on these projects. Trigger condition is narrow by design: "programmable, on-chain, or cryptographically verifiable mechanisms" — traditional open-source civic tech does not qualify.
...
- The 70-point cap for moderate dossiers (completeness 0.35–0.6) is a conservative heuristic — a genuinely strong fit for C1–C3 could theoretically yield 60+ on criteria alone, so the cap is rarely binding for well-evidenced projects.

### Clear winner profile

A project that would clearly win under this constitution: a DAO or community-owned governance tool with on-chain transparency mechanisms, serving communities excluded from traditional financial institutions, with published governance model and open standards, actively deployed, with explicit attention to how its architecture distributes or concentrates power. Example: a cooperative-owned municipal budget transparency platform using verifiable data that local community groups can audit and challenge.

### Clear loser profile

A project that would clearly lose: a proprietary civic engagement platform (even well-funded, even well-documented) that aggregates citizen data for government use, has opaque governance, and serves primarily to make government administrative processes more efficient without redistributing power or increasing accountability to citizens.
