...
**16 projects** — the constitutional #1 pick of each of the 17 agents (David Powell and Francesca Galli independently chose the same project, yielding 16 unique candidates).

Selection rationale: the candidate set is defined entirely by what each agent actually champions under their own constitution, with no input from committee-level aggregation (v11–v14 PRs). Every agent has exactly one candidate in the room. The social choice problem is: given that all 17 agents' top picks are on the table, what does the committee choose?

| ID | Name | URL | Mean Rank (v11) | Constitutional champion(s) |
|----|------|-----|-----------------|---------------------------|
| `algorithm-watch` | AlgorithmWatch | https://algorithmwatch.org | #39 | Safeguard |
| `bonfire` | Bonfire | https://bonfirenetworks.org | #7 | Gamithra Marga |
| `decidim` | Decidim | https://decidim.org | #3 | Alessandro Pedori |
| `ethelo` | Ethelo | https://ethelo.com | #54 | Huda Abdirahim |
| `gapminder-upgrader` | Gapminder Worldview Upgrader | https://upgrader.gapminder.org | #99 | Prism (Tuna Acisu) |
...
| `liquidfeedback` | LiquidFeedback | https://liquidfeedback.com | #1 | Davit Jintcharadze |
| `martus` | Martus | https://www.martus.org | #49 | Signal (Martina Orlea) |
| `mastodon` | Mastodon | https://github.com/mastodon/mastodon | #21 | Fatima Sarah Khalid (v5) |
| `mysociety-data` | mySociety Datasets and APIs | https://data.mysociety.org | #2 | David Powell + Francesca Galli |
| `ohm` | Open Heart Mind | https://openheartmind.org | #28 | Beacon (Frederick O'Brien) |
| `opencrvs` | OpenCRVS | https://www.opencrvs.org | #25 | Aadi Kulkarni |
...
* **AlgorithmWatch** (`algorithm-watch`) — Research and advocacy organisation monitoring algorithmic accountability, AI transparency, and automated decision-making in public sector and corporate contexts.
...
* **Gapminder Worldview Upgrader** (`gapminder-upgrader`) — Interactive fact-checking quizzes about global development data. Designed to surface and correct systematic cognitive biases about poverty, health, and global progress.
...
**Scoring:** With 16 candidates, the top-ranked candidate receives 15 points, second receives 14, ..., last-ranked receives 0.

**Winner computation:**
1. Sum each candidate's Borda points across all 17 ballots
2. Maximum possible score: 15 × 17 = 255 points
...
The 16 candidates are defined solely by the 17 agents' constitutional #1 picks — no committee-level aggregation (v11–v14) was used to construct this set. This is a deliberate design choice: the social choice problem is to find what the committee chooses when every agent's most-valued project is on the table.

**Structural features of this shortlist:**

Mean rank (v11) spans from #1 (LiquidFeedback) to #157 (Worker Info Exchange), with a median around #30. This wide spread means committee-aggregate consensus and individual constitutional conviction are genuinely in tension across the candidate set.

- **High mean rank** — LiquidFeedback (#1), mySociety Datasets and APIs (#2), Decidim (#3), Polis (#10): well-supported across agents, likely Borda frontrunners under honest voting
- **Mid-range** — Mastodon (#21), OpenCRVS (#25), Open Heart Mind (#28): strong individual champions, plausible coalition targets
- **Low mean rank** — AlgorithmWatch (#39), Martus (#49), Ethelo (#54), Interop. Delib. (#72), Humble Data (#87), Gapminder (#99), Worker Info Exchange (#157): high individual conviction, low committee consensus; their champions face the strongest strategic pressure to deviate from honest ballots

The deliberative question: do the 8 agents with low-ranked constitutional winners converge on LiquidFeedback as a second-best, or can they form a coalition around an alternative that beats it?
