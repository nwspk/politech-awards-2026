#!/usr/bin/env python3
"""Run Gemini jury calls for Aadi Kulkarni using gemini-2.5-flash (gemini-pro-1.5 unavailable)."""
...
def call_gemini(run_num):
    ...
    data = {
        ...
        "note": "google/gemini-pro-1.5 unavailable on OpenRouter; used google/gemini-2.5-flash as substitute"
    }

    out_path = os.path.join(LOG_DIR, f"gemini-run-{run_num}.json")
    with open(out_path, "w") as f:
        json.dump(data, f, indent=2)

    print(f"  Saved gemini-run-{run_num}.json | Winner: {parsed_winner}")
    print(f"  Preview: {raw_response[:300]}")
    return parsed_winner


if __name__ == "__main__":
    for run in range(1, 4):
        print(f"\nRunning Gemini run {run}...")
        winner = call_gemini(run)
        time.sleep(2)
