#!/usr/bin/env python3
"""Jury runner for Huda Abdirahim — Project A pipeline."""
...
def call_anthropic(run_num, log_dir):
    """Call Claude Sonnet 4.6 directly via Anthropic API."""
    import urllib.request
    ...
    path = os.path.join(log_dir, f"claude-run-{run_num}.json")
    with open(path, "w") as f:
        json.dump(log, f, indent=2)
    print(f"[claude-run-{run_num}] winner: {log['parsed_winner']}")
    return log
...
def extract_winner(text):
    """Try to extract the project name from the response."""
    import re
    # Look for bold project names or "Winner:" patterns
    patterns = [
        r'\*\*Winner[:\s]+([^*\n]+)\*\*',
        r'Winner[:\s]+\*\*([^*\n]+)\*\*',
        r'winner[:\s]+\*\*([^*\n]+)\*\*',
        ...
        r'choose\s+\*\*([^*\n]+)\*\*',
        r'pick\s+\*\*([^*\n]+)\*\*',
        r'^#+\s+(?:Winner|My (?:pick|choice|winner)[:\s]*)([^\n]+)',
    ...
    # Check for common project names in the first 300 chars
    projects = [
...
def main():
    ...
    for run in range(1, 4):
        try:
            log = call_anthropic(run, log_dir)
            all_logs.append(log)
            time.sleep(1)
        except Exception as e:
            print(f"[claude-run-{run}] ERROR: {e}")
    ...
    grid = {}
    for log in all_logs:
        winner = log["parsed_winner"]
        votes[winner] += 1
        key = f"{log['model']}-run-{log['run']}"
        grid[key] = winner

    print("\n=== VOTE TALLY ===")
    for proj, count in votes.most_common():
        print(f"  {proj}: {count}/15 ({count/15*100:.0f}%)")
